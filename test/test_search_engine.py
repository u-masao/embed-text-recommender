...
from src.models.search_engine import DummySearchEngine, SearchEngine
...
@pytest.fixture(
    scope="session",
    params=[
        (SearchEngine(DummySearchEngine(dimension=EMBEDDING_DIMENSION))),
    ],
)
def engine(request):
    """
...
def test_basic(engine, ids_embeds, top_n):
    # 初期化チェック
    assert engine is not None, "Embedding が正しく初期化されていません"
    ...
    if embeds.ndim == 1:
        # ベクトルの場合
        assert result_ids.shape[0] == top_n
        assert similarities.shape[0] == top_n
    elif embeds.ndim == 2:
        # 行列の場合
